...
double mappers::TechSchool::calculateComparisonScore(const double armyInvestment,
...
{
	return std::abs(armyInvestment - armyBonus - 0.2) + std::abs(navyInvestment - navyBonus - 0.2) + std::abs(commerceInvestment - commerceBonus - 0.2) +
			 std::abs(industryInvestment - industryBonus - 0.2) + std::abs(cultureInvestment - cultureBonus - 0.2);
}
