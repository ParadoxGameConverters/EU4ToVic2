...
void V2::World::outputGTFO(std::map<std::string, std::shared_ptr<V2::Country>> countries) const
{
	...
	for (const auto& country: countries)
	{
		...
		output << "\t\tallow = {\n";
		output << "\t\t\twar = no\n";
		output << "\t\t\t" << country.first << " = { war = no }\n";
		output << "\t\t}\n";
		output << "\n";
		output << "\t\teffect = {\n";
		output << "\t\t\tprestige_factor = -0.1\n";
		output << "\t\t\trelease_vassal = " << country.first << "\n";
		output << "\t\t\tdiplomatic_influence = { who = " << country.first << " value = -100 }\n";
		output << "\t\t}\n";
		output << "\t}\n";

		outLoc << "gtfo_" << country.first << "_title;Grant " << countryName << " Independence;;;;;;;;;;;;x,,\n";
		outLoc << "gtfo_" << country.first << "_desc;We've had a good run, but it's time to end our rulership of " << countryName
...
void V2::World::outputReturnCores(std::map<std::string, std::shared_ptr<V2::Country>> countries) const
{
	...
	for (const auto& country: countries)
	{
		...
		output << "\n";
		output << "\treturn_cores_" << country.first << " = {\n";
		output << "\t\tpicture = return_cores_img\n";
		output << "\t\talert = no\n";
		output << "\n";
		output << "\t\tpotential = {\n";
		...
		output << "\t\teffect = {\n";
		output << "\t\t\tprestige = -5\n";
		output << "\t\t\trelation = { who = " << country.first << " value = 50 }\n";
		output << "\t\t\tany_owned = {\n";
		output << "\t\t\t\tlimit = {\n";
		output << "\t\t\t\t\tNOT = { is_core = THIS }\n";
		output << "\t\t\t\t\tis_core = " << country.first << "\n";
		output << "\t\t\t\t}\n";
		output << "\t\t\t\tsecede_province = " << country.first << "\n";
		output << "\t\t\t}\n";
		output << "\t\t}\n";
		output << "\t}\n";

		outLoc << "return_cores_" << country.first << "_title;Return cores to " << countryName << ";;;;;;;;;;;;;;;;;;;;x,,\n";
		outLoc << "return_cores_" << country.first << "_desc;We hold provinces that " << countryName
